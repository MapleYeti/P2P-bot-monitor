...
info(
  `Bot chat webhook: ${
    configSummary.botChatWebhookConfigured ? "Configured" : "Not configured"
  }`,
  "Server"
);
info(
  `Monitoring ${configSummary.botCount} bots: ${
    configSummary.botNames.join(", ") || "None"
  }`,
  "Server"
);
...
watcher
  .on(FILE_OPERATIONS.ADD, (filePath) => {
    try {
      ...
      error(
        `Error processing new file ${filePath}: ${err.message}`,
        "FileWatcher"
      );
    }
  })
  .on(FILE_OPERATIONS.CHANGE, async (filePath) => {
    try {
      ...
      error(
        `Error processing file change ${filePath}: ${err.message}`,
        "FileWatcher"
      );
    }
  })
